  ...
  return (
    <>
      ...
      <div
        ...
        onClick={toggleSidebar}
      />
      <div className={`${styles.sidebar} ${isSidebarOpen ? styles.sidebarOpen : ""} ${isLight ? styles.sidebarLight : ""}`}>
        ...
        <nav className={`${styles.sidebarNav} ${isLight ? styles.navLight : ""}`}
    aria-label="Primary navigation">
              {navItems.map((item) =>
                item.label === "SERVICES" && servicesDropdown.length > 0 ? (
                  <div key={item.id} className={styles.sidebarNavItemWithMenu}>
                    ...
                    >
                      {item.label}
                      <span className={`${styles.arrow} ${isServicesExpanded ? styles.arrowExpanded : ""}`}>▾</span>
                    </button>
                    <div className={`${styles.sidebarSubmenu} ${isServicesExpanded ? styles.sidebarSubmenuExpanded : ""}`}>
                      {servicesDropdown.map((service) => (
                        <Link
                          key={service.id}
                          href={service.href}
