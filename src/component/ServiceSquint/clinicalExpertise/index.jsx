...
const SquintClinicalExpertise = () => {
  const { clinicalExpertise: data } = SERVICE_SQUINT_CONTENT;
  ...
  return (
    <section
      ref={sectionRef}
      className={`${styles["clinical-expertise"]} ${isRevealed ? styles["is-revealed"] : ""
        }`.trim()}
      aria-labelledby="squint-clinical-expertise-title"
    >
      <div className={styles["clinical-expertise__inner"]}>
        <div className={styles["clinical-expertise__header"]}>
          <h2 id="squint-clinical-expertise-title">{data.title}</h2>
          <p>{data.intro}</p>
        </div>
